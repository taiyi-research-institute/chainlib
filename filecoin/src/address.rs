...
impl libAddress for FilecoinAddress {
    ...
    fn from_public_key(public_key: &Self::PublicKey, _: &Self::Format) -> Result<Self, AddressError> {
        ...
        let addr = Address::new_secp256k1(&stream).unwrap();
        Ok(FilecoinAddress(encode(&addr)))
    }
}
...
mod tests {
    ...
    fn base32_to_internal_address() {
        let addr = Address::from_str("f2qexjxohk7c7j6r2tud6kgab6yd62fhdszjukcra").unwrap();
        println!("{}", addr);
    }
