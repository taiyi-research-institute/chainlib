...
/// Represents a Omni address
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct OmniAddress<N: OmniNetwork> {
    /// The Omni address
    address: String,
...
impl<N: OmniNetwork> Address for OmniAddress<N> {
    ...
    fn from_private_key(private_key: &Self::PrivateKey, format: &Self::Format) -> Result<Self, AddressError> {
        ...
        match format {
            ...
            OmniFormat::P2SH_P2WPKH => Self::p2sh_p2wpkh(&public_key),
            OmniFormat::Bech32 => Self::bech32(&public_key),
        }
    }

    /// Returns the address corresponding to the given Omni public key.
...
impl<N: OmniNetwork> OmniAddress<N> {
    /// Returns a P2PKH address from a given Omni public key.
    pub fn p2pkh(public_key: &<Self as Address>::PublicKey) -> Result<Self, AddressError> {
        ...
        address[0] = N::to_address_prefix(&OmniFormat::P2PKH)[0];
        ...
        Ok(Self {
            ...
            format: OmniFormat::P2PKH,
            _network: PhantomData,
        })
    }

    // Returns a P2WSH address in Bech32 format from a given Omni script
    pub fn p2wsh(original_script: &Vec<u8>) -> Result<Self, AddressError> {
        let script = Sha256::digest(&original_script).to_vec();

        // Organize as a hash
        let v = N::to_address_prefix(&OmniFormat::P2WSH)[0];
        ...
        //let bech32 = Bech32::new(String::from_utf8(N::to_address_prefix(&OmniFormat::Bech32))?, data)?;
        let prefix = String::from_utf8(N::to_address_prefix(&OmniFormat::Bech32))?;
        let bech32 = bech32::encode(&prefix, data, Variant::Bech32)?;
        Ok(Self {
            address: bech32,
            format: OmniFormat::P2WSH,
            _network: PhantomData,
        })
    }

    /// Returns a P2SH_P2WPKH address from a given Omni public key.
    pub fn p2sh_p2wpkh(public_key: &<Self as Address>::PublicKey) -> Result<Self, AddressError> {
        let mut address = [0u8; 25];
        address[0] = N::to_address_prefix(&OmniFormat::P2SH_P2WPKH)[0];
        ...
        Ok(Self {
            address: address.to_base58(),
            format: OmniFormat::P2SH_P2WPKH,
            _network: PhantomData,
        })
    }

    /// Returns a Bech32 address from a given Omni public key.
    pub fn bech32(public_key: &<Self as Address>::PublicKey) -> Result<Self, AddressError> {
        ...
        Ok(Self {
            ...
            format: OmniFormat::Bech32,
            _network: PhantomData,
        })
    }

    /// Returns the format of the Omni address.
    pub fn format(&self) -> OmniFormat {
        self.format.clone()
    }

    /// Returns a redeem script for a given Omni public key.
...
impl<'a, N: OmniNetwork> TryFrom<&'a str> for OmniAddress<N> {
...
impl<N: OmniNetwork> FromStr for OmniAddress<N> {
    ...
    fn from_str(address: &str) -> Result<Self, Self::Err> {
        ...
        if let Ok(format) = OmniFormat::from_address_prefix(prefix.as_bytes()) {
            if OmniFormat::Bech32 == format {
        ...
        let format = OmniFormat::from_address_prefix(&data[0..2])?;
...
impl<N: OmniNetwork> fmt::Display for OmniAddress<N> {
...
mod tests {
    ...
    fn test_from_private_key<N: OmniNetwork>(
        expected_address: &str,
        private_key: &OmniPrivateKey<N>,
        format: &OmniFormat,
    ) {
        let address = OmniAddress::from_private_key(private_key, format).unwrap();
        assert_eq!(expected_address, address.to_string());
    }

    fn test_from_public_key<N: OmniNetwork>(
        expected_address: &str,
        public_key: &OmniPublicKey<N>,
        format: &OmniFormat,
    ) {
        let address = OmniAddress::from_public_key(public_key, format).unwrap();
        assert_eq!(expected_address, address.to_string());
    }

    fn test_from_str<N: OmniNetwork>(expected_address: &str, expected_format: &OmniFormat) {
        let address = OmniAddress::<N>::from_str(expected_address).unwrap();
        assert_eq!(expected_address, address.to_string());
        assert_eq!(*expected_format, address.format);
    }

    fn test_to_str<N: OmniNetwork>(expected_address: &str, address: &OmniAddress<N>) {
        assert_eq!(expected_address, address.to_string());
    }
    ...
    mod p2pkh_mainnet_compressed {
        ...
        fn from_private_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = OmniPrivateKey::<N>::from_str(private_key).unwrap();
                test_from_private_key(address, &private_key, &OmniFormat::P2PKH);
            });
        }
        ...
        fn from_public_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = OmniPrivateKey::<N>::from_str(private_key).unwrap();
                let public_key = OmniPublicKey::<N>::from_private_key(&private_key);
                test_from_public_key(address, &public_key, &OmniFormat::P2PKH);
            });
        }
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, address)| {
                test_from_str::<N>(address, &OmniFormat::P2PKH);
            });
        }
    ...
    mod p2pkh_mainnet_uncompressed {
        ...
        fn test_invalid() {
            ...
            let private_key = OmniPrivateKey::<N>::from_str(private_key).unwrap();
            let address = OmniAddress::<N>::from_private_key(&private_key, &OmniFormat::P2PKH).unwrap();
            assert_ne!(expected_address, address.to_string());

            let public_key = OmniPublicKey::<N>::from_private_key(&private_key);
            let address = OmniAddress::<N>::from_public_key(&public_key, &OmniFormat::P2PKH).unwrap();
            assert_ne!(expected_address, address.to_string());

            // Invalid address length

            let address = "1";
            assert!(OmniAddress::<N>::from_str(address).is_err());
    ...
    mod p2sh_p2wpkh_mainnet {
        ...
        fn from_private_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = OmniPrivateKey::<N>::from_str(private_key).unwrap();
                test_from_private_key(address, &private_key, &OmniFormat::P2SH_P2WPKH);
            });
        }
        ...
        fn from_public_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = OmniPrivateKey::<N>::from_str(private_key).unwrap();
                let public_key = OmniPublicKey::<N>::from_private_key(&private_key);
                test_from_public_key(address, &public_key, &OmniFormat::P2SH_P2WPKH);
            });
        }
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, address)| {
                test_from_str::<N>(address, &OmniFormat::P2SH_P2WPKH);
            });
        }
        ...
        fn test_invalid() {
            ...
            let private_key = OmniPrivateKey::<N>::from_str(private_key).unwrap();
            let address = OmniAddress::<N>::from_private_key(&private_key, &OmniFormat::P2SH_P2WPKH).unwrap();
            assert_ne!(expected_address, address.to_string());

            let public_key = OmniPublicKey::<N>::from_private_key(&private_key);
            let address = OmniAddress::<N>::from_public_key(&public_key, &OmniFormat::P2SH_P2WPKH).unwrap();
            assert_ne!(expected_address, address.to_string());
    ...
    mod bech32_mainnet {
        ...
        fn from_invalid_address() {
            INVALID.iter().for_each(|invalid_bech32| {
                assert_eq!(true, OmniAddress::<N>::from_str(invalid_bech32).is_err());
            });
        }
        ...
        fn from_private_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = OmniPrivateKey::<N>::from_str(private_key).unwrap();
                test_from_private_key(address, &private_key, &OmniFormat::Bech32);
            });
        }
        ...
        fn from_public_key() {
            KEYPAIRS.iter().for_each(|(private_key, address)| {
                let private_key = OmniPrivateKey::<N>::from_str(private_key).unwrap();
                let public_key = OmniPublicKey::<N>::from_private_key(&private_key);
                test_from_public_key(address, &public_key, &OmniFormat::Bech32);
            });
        }
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, address)| {
                test_from_str::<N>(address, &OmniFormat::Bech32);
            });
        }
    ...
    mod p2wsh_mainnet {
        ...
        fn test_addr(){
            ...
            let new_address = OmniAddress::<N>::p2wsh(&script_hex).unwrap();
    ...
    mod p2wsh_testnet {
        ...
        fn from_str() {
            SCRIPTPAIRS.iter().for_each(|(script, address)| {
                ...
                assert_eq!(new_address.format, OmniFormat::P2WSH);
            });
        }
