...
/// Represents the amount of Omni in satoshis
#[derive(Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize)]
pub struct OmniAmount(pub i64);
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            match self {
                ...
                Denomination::Omni => "BTC",
            }
        )
    }
}

impl Amount for OmniAmount {}

impl OmniAmount {
    /// The zero amount.
    pub const ZERO: OmniAmount = OmniAmount(0);
    /// Exactly one satoshi.
    pub const ONE_SAT: OmniAmount = OmniAmount(1);
    /// Exactly one omni.
    pub const ONE_BTC: OmniAmount = OmniAmount(COIN);
    ...
    pub fn from_btc(btc_value: i64) -> Result<Self, AmountError> {
        let satoshis = btc_value * 10_i64.pow(Denomination::Omni.precision());
    ...
    pub fn sub(self, b: OmniAmount) -> Result<Self, AmountError> {
        Self::from_satoshi(self.0 - b.0)
    }
}

impl fmt::Display for OmniAmount {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
mod tests {
    ...
    fn test_from_satoshi(sat_value: i64, expected_amount: OmniAmount) {
        let amount = OmniAmount::from_satoshi(sat_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_ubtc(ubtc_value: i64, expected_amount: OmniAmount) {
        let amount = OmniAmount::from_ubtc(ubtc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_mbtc(mbtc_value: i64, expected_amount: OmniAmount) {
        let amount = OmniAmount::from_mbtc(mbtc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_cbtc(cbtc_value: i64, expected_amount: OmniAmount) {
        let amount = OmniAmount::from_cbtc(cbtc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_dbtc(dbtc_value: i64, expected_amount: OmniAmount) {
        let amount = OmniAmount::from_dbtc(dbtc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_from_btc(btc_value: i64, expected_amount: OmniAmount) {
        let amount = OmniAmount::from_btc(btc_value).unwrap();
        assert_eq!(expected_amount, amount)
    }

    fn test_addition(a: &i64, b: &i64, result: &i64) {
        let a = OmniAmount::from_satoshi(*a).unwrap();
        let b = OmniAmount::from_satoshi(*b).unwrap();
        let result = OmniAmount::from_satoshi(*result).unwrap();
    ...
    mod valid_conversions {
        ...
        fn test_satoshi_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_satoshi(amounts.satoshi, OmniAmount(amounts.satoshi)));
        }
        ...
        fn test_ubtc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_ubtc(amounts.micro_bit, OmniAmount(amounts.satoshi)));
        }
        ...
        fn test_mbtc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_mbtc(amounts.milli_bit, OmniAmount(amounts.satoshi)));
        }
        ...
        fn test_cbtc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_cbtc(amounts.centi_bit, OmniAmount(amounts.satoshi)));
        }
        ...
        fn test_dbtc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_dbtc(amounts.deci_bit, OmniAmount(amounts.satoshi)));
        }
        ...
        fn test_btc_conversion() {
            TEST_AMOUNTS
                .iter()
                .for_each(|amounts| test_from_btc(amounts.omni, OmniAmount(amounts.satoshi)));
        }
    }
