...
/// Represents the format of a Omni address
...
impl Format for OmniFormat {}

impl OmniFormat {
    /// Returns the address prefix of the given network.
    pub fn to_address_prefix<N: OmniNetwork>(&self) -> Vec<u8> {
    ...
    pub fn from_address_prefix(prefix: &[u8]) -> Result<Self, AddressError> {
        ...
        match (prefix[0], prefix[1]) {
            (0x00, _) | (0x6F, _) => Ok(OmniFormat::P2PKH),
            (0x05, _) | (0xC4, _) => Ok(OmniFormat::P2SH_P2WPKH),
            (0x62, 0x63) | (0x74, 0x62) => Ok(OmniFormat::Bech32),
            _ => return Err(AddressError::InvalidPrefix(String::from_utf8(prefix.to_vec())?)),
        }
    }
}

impl fmt::Display for OmniFormat {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            OmniFormat::P2PKH => write!(f, "p2pkh"),
            OmniFormat::P2WSH => write!(f, "p2wsh"),
            OmniFormat::P2SH_P2WPKH => write!(f, "p2sh_p2wpkh"),
            OmniFormat::Bech32 => write!(f, "bech32"),
        }
    }
}
