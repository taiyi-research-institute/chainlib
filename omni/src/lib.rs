...
mod testnet_daemon{
    ...
    #[test]
    fn create_account(){
        let priv_key = OmniPrivateKey::<N>::new(&mut thread_rng()).unwrap();
        println!("{}",priv_key.to_string());
        let address = priv_key.to_address(&OmniFormat::P2PKH).unwrap();
        println!("{}",address)
    }
    ...
    fn send_p2pkh() {
        ...
        let address1 = OmniAddress::<N>::from_str("mhSWVCZ7GtrYeDavBbZCUKownLPSAnxMyD").unwrap();
        let input = OmniTransactionInput::<N>::new(txid, 1, None, None, None, create_script_pub_key(&address1).ok(), None, SignatureHash::SIGHASH_ALL).unwrap();
        let recv_address = OmniAddress::<N>::from_str("miByfZ8aBt8xQwUW9DXJw4ockykf2P42MZ").unwrap();
        let amount = OmniAmount(50000);
        let output = OmniTransactionOutput::new(&recv_address, amount).unwrap();
    }
}