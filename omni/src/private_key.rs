...
use chainlib_core::{crypto::checksum, Address, AddressError, PrivateKey, PrivateKeyError, PublicKey, libsecp256k1};

use base58::{FromBase58, ToBase58};
use core::{fmt, fmt::Display, marker::PhantomData, str::FromStr};
use rand::Rng;

/// Represents a Omni private key
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct OmniPrivateKey<N: OmniNetwork> {
...
impl<N: OmniNetwork> PrivateKey for OmniPrivateKey<N> {
    type Address = OmniAddress<N>;
    type Format = OmniFormat;
    type PublicKey = OmniPublicKey<N>;

    /// Returns a randomly-generated compressed Omni private key.
    fn new<R: Rng>(rng: &mut R) -> Result<Self, PrivateKeyError> {
        Ok(Self {
            secret_key: libsecp256k1::SecretKey::random(rng),
            compressed: true,
            _network: PhantomData,
        })
    }

    /// Returns the public key of the corresponding Omni private key.
    fn to_public_key(&self) -> Self::PublicKey {
        Self::PublicKey::from_private_key(self)
    }

    /// Returns the address of the corresponding Omni private key.
    fn to_address(&self, format: &Self::Format) -> Result<Self::Address, AddressError> {
        Self::Address::from_private_key(self, format)
    }
}

impl<N: OmniNetwork> OmniPrivateKey<N> {
    /// Returns a private key given a secp256k1 secret key.
    pub fn from_secp256k1_secret_key(secret_key: &libsecp256k1::SecretKey, compressed: bool) -> Self {
        Self {
...
impl<N: OmniNetwork> FromStr for OmniPrivateKey<N> {
    type Err = PrivateKeyError;

    /// Returns a Omni private key from a given WIF.
    fn from_str(wif: &str) -> Result<Self, Self::Err> {
        ...
        Ok(Self {
            secret_key: libsecp256k1::SecretKey::parse_slice(&data[1..33])?,
            compressed: len == 38,
            _network: PhantomData,
        })
    }
...
impl<N: OmniNetwork> Display for OmniPrivateKey<N> {
...
mod tests {
    ...
    fn test_to_public_key<N: OmniNetwork>(
        expected_public_key: &OmniPublicKey<N>,
    ...
    fn test_to_address<N: OmniNetwork>(
        expected_address: &OmniAddress<N>,
        expected_format: &OmniFormat,
        private_key: &OmniPrivateKey<N>,
    ) {
    ...
    fn test_from_secp256k1_secret_key<N: OmniNetwork>(
        expected_wif: &str,
        ...
        let private_key = OmniPrivateKey::<N>::from_secp256k1_secret_key(&secret_key, compressed);
    ...
    fn test_from_str<N: OmniNetwork>(
        ...
        let private_key = OmniPrivateKey::<N>::from_str(wif).unwrap();
    ...
    fn test_to_str<N: OmniNetwork>(expected_private_key: &str, private_key: &OmniPrivateKey<N>) {
    ...
    mod p2pkh_mainnet_compressed {
        ...
        fn to_public_key() {
            KEYPAIRS.iter().for_each(|(private_key, public_key, _)| {
                let public_key = OmniPublicKey::<N>::from_str(public_key).unwrap();
        ...
        fn to_address() {
            KEYPAIRS.iter().for_each(|(private_key, _, address)| {
                let address = OmniAddress::<N>::from_str(address).unwrap();
                let private_key = OmniPrivateKey::<N>::from_str(&private_key).unwrap();
                test_to_address(&address, &OmniFormat::P2PKH, &private_key);
            });
        }
        ...
        fn from_secp256k1_secret_key() {
            KEYPAIRS
                ...
                .for_each(|(expected_private_key, expected_public_key, expected_address)| {
                    let private_key = OmniPrivateKey::<N>::from_str(&expected_private_key).unwrap();
        ...
        fn from_str() {
            KEYPAIRS
                ...
                .for_each(|(private_key, expected_public_key, expected_address)| {
                    let expected_private_key = OmniPrivateKey::<N>::from_str(&private_key).unwrap();
                    test_from_str::<N>(
        ...
        fn test_invalid() {
            ...
            let private_key = "L";
            assert!(OmniPrivateKey::<N>::from_str(private_key).is_err());
    ...
    mod p2sh_p2wpkh_mainnet {
        ...
        fn to_address() {
            KEYPAIRS.iter().for_each(|(private_key, _, address)| {
                let address = OmniAddress::<N>::from_str(address).unwrap();
                let private_key = OmniPrivateKey::<N>::from_str(&private_key).unwrap();
                test_to_address(&address, &OmniFormat::P2SH_P2WPKH, &private_key);
            });
        }
