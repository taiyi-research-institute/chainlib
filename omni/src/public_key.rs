...
use chainlib_core::{Address, AddressError, PublicKey, PublicKeyError, libsecp256k1};

use core::{fmt, fmt::Display, marker::PhantomData, str::FromStr};


/// Represents a Omni public key
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct OmniPublicKey<N: OmniNetwork> {
...
impl<N: OmniNetwork> PublicKey for OmniPublicKey<N> {
    ...
    fn from_private_key(private_key: &Self::PrivateKey) -> Self {
        Self {
            public_key: libsecp256k1::PublicKey::from_secret_key(&private_key.to_secp256k1_secret_key()),
...
impl<N: OmniNetwork> OmniPublicKey<N> {
...
impl<N: OmniNetwork> FromStr for OmniPublicKey<N> {
...
impl<N: OmniNetwork> Display for OmniPublicKey<N> {
...
mod tests {
    ...
    fn test_from_private_key<N: OmniNetwork>(
        ...
        let public_key = OmniPublicKey::from_private_key(private_key);
    ...
    fn test_from_str<N: OmniNetwork>(
        ...
        let public_key = OmniPublicKey::<N>::from_str(expected_public_key).unwrap();
        let address = public_key.to_address(expected_format).unwrap();
    ...
    fn test_to_str<N: OmniNetwork>(expected_public_key: &str, public_key: &OmniPublicKey<N>) {
        assert_eq!(expected_public_key, public_key.to_string());
    }
    ...
    mod p2pkh_mainnet_compressed {
        ...
        fn to_address() {
            KEYPAIRS.iter().for_each(|(_, public_key, address)| {
                ...
                let public_key = OmniPublicKey::<N>::from_str(&public_key).unwrap();
                test_to_address(&address, &OmniFormat::P2PKH, &public_key);
            });
        }
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, expected_public_key, expected_address)| {
                test_from_str::<N>(expected_public_key, expected_address, true, &OmniFormat::P2PKH);
            });
        }
    ...
    mod p2pkh_mainnet_uncompressed {
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, expected_public_key, expected_address)| {
                test_from_str::<N>(expected_public_key, expected_address, false, &OmniFormat::P2PKH);
            });
        }
    ...
    mod p2sh_p2wpkh_mainnet {
        ...
        fn to_address() {
            KEYPAIRS.iter().for_each(|(_, public_key, address)| {
                let address = OmniAddress::<N>::from_str(address).unwrap();
                let public_key = OmniPublicKey::<N>::from_str(&public_key).unwrap();
                test_to_address(&address, &OmniFormat::P2SH_P2WPKH, &public_key);
            });
        }
        ...
        fn from_str() {
            KEYPAIRS.iter().for_each(|(_, expected_public_key, expected_address)| {
                test_from_str::<N>(expected_public_key, expected_address, true, &OmniFormat::P2SH_P2WPKH);
            });
        }
    ...
    fn test_p2pkh_invalid() {
        ...
        let public_key = "0";
        assert!(OmniPublicKey::<N>::from_str(public_key).is_err());
