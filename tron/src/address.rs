use chainlib_core::{Address,AddressError, PrivateKey};
use crate::{TronFormat,TronPrivateKey,TronPublicKey};
use serde::Serialize;
use sha3::Keccak256;
use hex::FromHex;
use std::fmt;
...
mod tests{
    ...
    #[test]
    fn test_address_from_private(){
        let private = TronPrivateKey::from_str("427139B43028A492E2705BCC9C64172392B8DB59F3BA1AEDAE41C88924960091").unwrap();
        let addr = TronAddress::from_private_key(&private, &TronFormat::Standard).unwrap();
        let pubkey = TronPublicKey::from_private_key(&private);
        println!("{}",pubkey.to_string());
        println!("HEX:{}",addr.to_hex());
        assert_eq!(addr.to_base58(),"TDpBe64DqirkKWj6HWuR1pWgmnhw2wDacE");
        assert_eq!(addr.to_hex(),"412A2B9F7641D0750C1E822D0E49EF765C8106524B");
    }
